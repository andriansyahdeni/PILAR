  ...
  activeBox.innerHTML = reports.filter(r => r.status === 'proses').map(t => `
    <div class="bg-white p-5 rounded-2xl border border-kf-sky/20 shadow-sm space-y-3">
      ...
      <div class="flex gap-2 pt-1 border-t border-gray-100">
        ...
        <a href="chat.php" class="px-3 py-2 bg-kf-light border rounded-xl"><i data-lucide="message-square" class="w-3.5 h-3.5"></i></a>
      </div>
    </div>
  `).join('');
...
function selectManagerChatChannel(id) {
  ...
  document.getElementById('manager-chat-empty-state').classList.add('hidden');
  document.getElementById('manager-chat-active-box').classList.remove('hidden');
  
  const r = reports.find(i => i.id === id);
  document.getElementById('manager-chat-box-title').textContent = `Saluran: ${r.title}`;
  renderManagerChatBoxMessages();
}

function renderManagerChatBoxMessages() {
  const container = document.getElementById('manager-chat-box-messages-container');
  container.innerHTML = (chatsData[currentChatReportId] || []).map(m => {
    const isMe = m.sender === 'Teknisi';
    return `<div class="flex ${isMe ? 'justify-end' : 'justify-start'}">
      <div class="${isMe ? 'bg-amber-600 text-white' : 'bg-kf-sky/40'} p-3 rounded-2xl text-xs max-w-[80%] shadow-2xs">
        <span class="block text-[8px] font-bold opacity-75 uppercase">${m.name}</span><p>${m.msg}</p>
      </div>
    </div>`;
  }).join('');
